...
if(sort_available($res->batint, $res->race, $Joueur->race))
{
	...
	{//On lance le sort
		...
		{//Déjà le sort
			$Message = bw_error("Vous avez déjà un sortilège de ".$Nom." en réserve.");
		}
		else 
		{	
			...
			{//On a passé
				...
				$Message = bw_info("Un Marteau D'Acier accompagne à présent vos unités jusqu'au ".date($CONF['game_timeformat'], $Temps_Insert)."!<br />\n");
			}
			else
			{
